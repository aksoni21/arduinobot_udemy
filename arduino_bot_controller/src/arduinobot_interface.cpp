#include "arduino_bot_controller/arduinobot_interface.hpp"
...
namespace arduinobot_controller
{
    ...
    CallbackReturn ArduinobotInterface::on_activate(const rclcpp_lifecycle::State &previous_state)
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while opening the connection with port " << port_);
            return CallbackReturn::FAILURE;
        }
        RCLCPP_INFO(rclcpp::get_logger("ArduinobotInterface"), "Hardware started, ready for commands...");
        return CallbackReturn::SUCCESS;
    };
    ...
    {
        if (prev_position_commands_ != position_commands_)
        {
            std::string msg;
            ...
            {
                RCLCPP_ERROR_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong while writing to the connection with port " << msg);
